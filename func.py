...
def record_db(file):
    """
    в функцию передаем json файл со стандартным набором слов на русском языке и вариантами переввода на английском языке,
    записываем все в бд 
    """
...
def random_words():
    """
    функция выводим на экран пользователя три не верных перевода слова 
    """
...
def word_ru(user_id=None):
    """
    в функцию передаем user_id, по нему получеам из бд слово на русском языке, если пользователь добавлял свои слова
    в бд, тогда будем брать случайное слово из двух таблиц
    """
...
def translator(word):
    """
    при помощи яндекс переводчика, узнаем перевод слова, которое получим из функции word_ru
    """
...
def added_user(id_user):
    """
    проверяем если пользователя еще нет в нашей бд, то записываем его. в функцию передаем его id
    """
...
def added_word(id_user=None, word=None):
    """
    в функцию передаем id и слово на русском языке и записываем в таблицу слов пользователя бд
    """
...
def delete_word(id_user=None, word=None):
    """
    удаляем слово которое ввел поьзователь из таблицы слов пользователя по id 
    """
    pk_user = session.query(Client.id).filter_by(id_client=id_user).first()
